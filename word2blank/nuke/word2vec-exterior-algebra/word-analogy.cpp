...
int main(int argc, char **argv) {
    ...
    while (1) {
        ...
        for (c = 0; c < words; c++) {
            ...
            dist = mulQuadForm(size, v.v, quadform, M[c].v, Ay, nullptr);
            float len1 = sqrtf(mulQuadForm(size, v.v, quadform, v.v, Ay, nullptr));
            float len2 = sqrtf(mulQuadForm(size, M[c].v, quadform, M[c].v, Ay, nullptr));
            dist /= len1;
            dist /= len2;
