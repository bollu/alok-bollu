...
int save_params(int nb_iter) {
    ...
    char *freq_irrelevant = (char *)malloc(sizeof(char) * MAX_STRING_LENGTH + 1);
    if (NULL == word) {
        return 1;
    }
    FILE *fid, *fout;
    ...
    // if (use_binary > 0 || nb_iter == 0) {
        ...
        assert(sizeof(realglove) == sizeof(double));
        ...
        for (a = 0; a < vocab_size; a++) {
            fscanf(fid, "%s %s", word, freq_irrelevant);
            // if (fscanf(fid,"%s",word) == 0) {free(word); fclose(fid); fclose(fout); return 1;}
            // // eat irrelevant frequency
            // if (fscanf(fid,"%s",word) == 0) { }
            ...
            for(b = 0; b < P; b++) {
                for (long long c = 0; c < vector_size; c++) {
                    fwrite(&syn0(c, b, a), sizeof(realglove), 1, fout);
                }
            }
            fprintf(fout, "\n");
        }
