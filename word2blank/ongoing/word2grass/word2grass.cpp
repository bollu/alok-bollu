    ...
    if (cbow) {  //train the cbow architecture
      ...
      for (a = b; a < window * 2 + 1 - b; a++) if (a != window) {
        ...
        // NEGATIVE SAMPLING
            ...
            arma::mat buff0(layer1_size, P); buff0.zeros();
            for (d = 0; d < negative + 1; d++) {
                if (d == 0) {
                    target = word;
                    //label = P*(layer1_size - P)/(2*layer1_size);
                    label = P;
                } else {
                    next_random = next_random * (unsigned long long)25214903917 + 11;
                    target = table[(next_random >> 16) % table_size];
                ...
                buff0 += (grad_syn0*g - (c_syn0.slice(last_word)*c_syn0.slice(last_word).t()*grad_syn0*g));
                arma::mat grad_neg = grad_syn1neg*g - (c_syn1neg.slice(target)*c_syn1neg.slice(target).t()*grad_syn1neg*g);
                c_syn1neg.slice(target) = exp_map(c_syn1neg.slice(target), grad_neg, 1.0);
            } // end negative samples loop
            c_syn0.slice(last_word) = exp_map(c_syn0.slice(last_word), buff0, 1.0);
