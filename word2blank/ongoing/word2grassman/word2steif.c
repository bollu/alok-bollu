    ...
    if (cbow) {  //train the cbow architecture
      ...
      for (a = b; a < window * 2 + 1 - b; a++) if (a != window) {
        ...
        if (negative > 0) for (d = 0; d < negative + 1; d++) {
          ...
          if (f > MAX_EXP) g = -2*(label - 1) * alpha;
          else if (f < -MAX_EXP) g = -2*(label - 0) * alpha;
          else g = -2*(label - expTable[(int)((f + MAX_EXP) * (EXP_TABLE_SIZE / MAX_EXP / 2))]) * (1 - expTable[(int)((f + MAX_EXP) * (EXP_TABLE_SIZE / MAX_EXP / 2))])*(expTable[(int)((f + MAX_EXP) * (EXP_TABLE_SIZE / MAX_EXP / 2))])*alpha;
