      ...
      for (a = b; a < window * 2 + 1 - b; a++) if (a != window) {
        ...
        if (negative > 0) for (d = 0; d < negative + 1; d++) {
          ...
          if (d == 0) {
            ...
            label = 0; // set target dot product to 0 because we want them to
                       // be aligned skew-symmetrically
          } else {
            // pick a random word
            next_random = next_random * (unsigned long long)25214903917 + 11;
            target = table[(next_random >> 16) % table_size];
            ...
            // set target dot product to 1 because we want them to be orthogonal
            // skew-symmetrically.
            label = 1;
          }
