...
function isValidForm(){
	...
	if (nbEquiv < 2) {
	  alert("No item 3 você deve fornecer pelo menos 2 palavras ou expressões equivalentes ou similares!");
	...
	if( equiv[0].textContent.trim() == equiv[1].textContent.trim() ){
	  alert("Forneça ao menos dois sinônimos DIFERENTES");
	  return false;
	}
...
function checkValid() {
  if( isValidForm() ) {
	...
	dbn.appendChild(document.createTextNode("Enviando..."));
	...
	dbp.appendChild(document.createTextNode("Enviando..."));
