...
std::map<std::string, Vec> word2negvec;
...
int main(int argc, char **argv) {
    char file_name[512];
    char file_name_neg[512];
    FILE *f, fneg;

    if (argc != 3) {
        printf("usage: %s <path-to-model.bin> <path-to-model-neg.bin>\n", argv[0]);
        return 0;
    }

    strcpy(file_name, argv[1]);
    strcpy(file_name_neg, argv[2]);
    printf("opening file:|%s|\n", file_name);
    fflush(stdout);

    f = fopen(file_name, "rb");
    fneg = fopen(file_name_neg, "rb");
    if (f == NULL) {
    ...
    if (fneg == NULL) {
        printf("Input negative file not found\n");
        return -1;
    }
    fscanf(f, "%lld", &words);
    fscanf(f, "%lld", &size);
    ...
    Mneg = (Vec *)malloc((long long)words * sizeof(Vec));
    Mnegl = (Vec *)malloc((long long)words * sizeof(Vec));
    Mnegloneminus = (Vec *)malloc((long long)words * sizeof(Vec));
    ...
    // Read fneg
    {
        int negwords, negsize;
        fscanf(fneg, "%lld", &negwords);
        fscanf(fneg, "%lld", &negsize);
        assert(negwords== words);
        assert(negsize== size);
    }
    ...
    for (long long b = 0; b < words; b++) {
        ...
        curword[a] = 0;
        assert (!strcmp(vocab + b *max_w, curword));
        Mneg[b] = new double[size];
        printf("\r%4lld / %4lld: %4.2f%%", b, words, 100.0 * ((float)b/words));
        ...
        for(int i = 0; i < size; ++i) {
            ...
            fread(&fl, sizeof(float), 1, f);
            Mneg[b][i] = fl;
            Mneg[b][i] = powf(2, M[b][i]);
        }
        word2negvec[std::string(vocab+b*max_w)] = M[b];
    }
