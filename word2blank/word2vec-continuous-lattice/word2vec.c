                ...
                if (a != window) {
                    ...
                    if (negative >= 0) {
                        for (d = 0; d < negative + 1; d++) {
                            ...
                            f = 0;
                            for (c = 0; c < layer1_size; c++) {
                                f += discrete(syn0[c + l1]) * discrete(syn1neg[c + l2]);
                            ...
                            total_loss += g * g;
                            for (c = 0; c < layer1_size; c++)
                                neu1e[c] = clamp(neu1e[c] + g * discrete(syn1neg[c + l2]));
                            for (c = 0; c < layer1_size; c++)
                               syn1neg[c + l2] = clamp(syn1neg[c + l2] + g * discrete(syn0[c + l1]));

                        } // loop around negsamples
                    } // if negative >= 0
                    ...
                    next_random =
                        ...
                        11;
                    for (c = 0; c < layer1_size; c++) {
                        syn0[c + l1] = clamp(syn0[c + l1] + neu1e[c]);
                    }
                } // if a != window
            } // end for loop around window
        // }
        sentence_position++;
...
void TrainModel() {
    ...
    if (classes == 0) {
        ...
        for (a = 0; a < vocab_size; a++) {
            ...
            if (binary)
                for (b = 0; b < layer1_size; b++) {
                    const real r = discrete(syn0[a * layer1_size + b]) > 0 ? 1 : -1; 
                    // const real r = syn0[a * layer1_size + b]; 
