...
struct Vec {
   ...
   public:
    ...
    inline void init(int len) {
        this->len = len;
        int a = posix_memalign((void **)&v, 128, (long long)len * sizeof(ff));
        assert(v != nullptr && "memory allocation failed");
        (void)a;
    }
    inline void initzero(int len) {
        this->len = len;
        this->v = (ff *)calloc(len, sizeof(ff));
    }
    Vec(int len, ff *v) : len(len), v(v) {}
    inline void set(int i, ff val) { v[i] = norm(val); }
    ...
    inline void accumscaleadd(ff g, const Vec &v2) const {
        g = norm(g);
        for (int i = 0; i < len; ++i) v[i] = norm(v[i] + g * v2.v[i]);
    }
    inline void pr() {
        const int npr = 10;
   ...
   public:
    ...
    inline void init(int outer, int inner) {
        ...
        int a = posix_memalign((void **)&m, 128,
                               (long long)outer * inner * sizeof(ff));
...
long long train_words = 0, word_count_actual = 0, iter = 5, file_size = 0,
          classes = 0;
real alpha = 0.0008, starting_alpha;
real sample = 1e-3;
...
void InitNet() {
    ...
    for (a = 0; a < vocab_size; a++)
        for (b = 0; b < layer1_size; b++) {
            ...
            ff CONST = (FFCOEFF / 2) / layer1_size;
            syn0.set(a, b,
                     (((next_random & 0xFFFF) / (real)65536) - 0.5) * CONST);
        }
