    ...
    while (1) {
        ...
        if ((!strcmp(st1, ":")) || (!strcmp(st1, "EXIT")) || feof(stdin)) {
            ...
            if (QID != 0) {
                ...
                printf(
                    "Total accuracy: %.2f %%   Semantic accuracy: %.2f %%   "
        ...
        if (b == words) continue;
        vec.fillzero();
        vec.accumscaleadd(1.0, M[b2]);
        vec.accumscaleadd(-1.0, M[b1]);
        vec.accumscaleadd(1.0, M[b3]);
        // for (a = 0; a < size; a++)
        //     vec[a] = (M[a + b2 * size] - M[a + b1 * size]) + M[a + b3 *
        //     size];
        TQS++;
        for (c = 0; c < words; c++) {
            if (c == b1) continue;
