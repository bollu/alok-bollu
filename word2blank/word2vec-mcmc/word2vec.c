...
// return min {2^x 1}. This is necessary, otherwise we will wind up calling
...
void *TrainModelThread(void *id) {
    ...
    char eof = 0;
    real fcur, fproposal, gcur, gproposal;
    clock_t now;
    real *neu1 = (real *)calloc(layer1_size, sizeof(real));
    real *neu1e = (real *)calloc(layer1_size, sizeof(real));
    real total_loss = 0;
    int nproposals = 0, nproposalsaccepted = 0;
    ...
    while (1) {
        if (word_count - last_word_count > 1000) {
            ...
            if ((debug_mode > 1)) {
                now = clock();
                printf(
                        "%cAlpha: %f  Progress: %.2f%%  Words/thread/sec: %.2fk  total_loss: %.2f Acceptance: %.2f%%",
                        13, alpha,
                        word_count_actual / (real)(iter * train_words + 1) * 100,
                        word_count_actual / ((real)(now - start + 1) /
                            (real)CLOCKS_PER_SEC * 1000),
                        total_loss,
                        100.0 * (real)nproposalsaccepted / (real)nproposals);
                total_loss = 0;
                nproposals = nproposalsaccepted = 0;
                fflush(stdout);
            }
        }
        ...
        for (a = b; a < window * 2 + 1 - b; a++)
            if (a != window) {
                ...
                // NEGATIVE SAMPLING
                // total noise used
                // noises[0] -> noise for focus
                // noises[1 + d] -> noise for the context word
                float noises[negative+2][layer1_size];
                for(int i = 0; i < negative + 2; ++i) {
                    for(int j = 0; j < layer1_size; ++j) {
                        next_random =
                            ...
                            11;
                        // sample noise
                        noises[i][j] = i > 0 ? 0 :  0.1 * (((next_random & 0xFFFF) / (real)65536) - 0.5);
                    }
                }

                // list of losses used
                float losscur = 0, lossproposal = 0;
                // list of indeces target words used
                ...
                for (d = 0; d < negative + 1; d++) {
                    if (d == 0) {
                        ...
                        targets[d] = target;
                        if (target == word) { targets[d] = -1; continue; }
                        label = 0;
                    }
                    l2 = target * layer1_size;
                    fcur = 0, fproposal = 0;
                    for (c = 0; c < layer1_size; c++) {
                        fcur += syn0[c + l1] * syn1neg[c + l2];
                        fproposal += (syn0[c + l1] + noises[0][c]) * 
                                     (syn1neg[c + l2] + noises[1 + d][c]);
                    }
                    ...
                    gproposal = (label - fproposal);
                    gproposals[d] = gproposal;

                    losscur += gcur * gcur;
                    lossproposal += gproposal * gproposal;

                } // end negative samples lop

                next_random =
                    next_random *
                    (unsigned long long)25214903917 +
                    11;
                const float r = (real)(next_random & 0xFFFF) / (real)65536;

                const float threshold = potMax1(losscur - lossproposal);
                // HACK HACK HACK
                const int accepted = lossproposal < losscur;


                total_loss += accepted == 1 ? lossproposal : losscur;
                nproposals++;
                nproposalsaccepted += accepted ? 1 : 0;

                // printf("%s | r: %f < %f, losscur: %f | lossproposal: %f\n", accepted ?
                ...
                if (accepted == 1) {
                    ...
                    for(int i = 0; i < negative + 1; ++i) {
                        ...
                        const unsigned l2 = targets[i] * layer1_size;
                        for(int j = 0; j < layer1_size; ++j) {
                            syn1neg[j + l2] += noises[1+i][j];
                        }
                    }
                }
                ...
                for(int i = 0; i < negative+1; ++i) {
                    if (targets[i] == -1) continue;
                    const float g = accepted ? gproposals[i] : gcurs[i];
                    const unsigned l2 = targets[i] * layer1_size;
                    for(int j = 0; j < layer1_size; ++j) {
                        syn1neg[j + l2] += g * syn0[j + l1] * alpha;
                    }
                }
                ...
                for(int i = 0; i < negative+1; ++i) {
                    ...
                    const unsigned l2 = targets[i] * layer1_size;
                    for(int j = 0; j < layer1_size; ++j) {
                        syn0[j + l1] += g * syn1neg[j + l2] * alpha;
                    }
                }
