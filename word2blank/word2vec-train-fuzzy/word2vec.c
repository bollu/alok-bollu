...
const float REGULARIZATION = 0.1; 
...
float clamp01(float x) {
    ...
    if (x < EPS) { return EPS; }
    else if (x >= 1.0 - EPS) { return 1.0 - EPS; }
    else { return x; }
}
...
// gradient of (0.5 - x)^2
void diff_uniform_grad(float *a, float g, float *agrad) {
    for(int i = 0; i < layer1_size; ++i) {
        agrad[i] = clamp01(agrad[i] + g * (0.5 - a[i]));
    }
}
...
// >>> h.diff(x)
// -log(x) + log(-x + 1) - 1 - (x - 1)/(-x + 1)
//= -log(x) + log(-x + 1) - 1 - (x - 1)/(1-x)
//= -log(x) + log(-x + 1) - 1 - (-1)
//= -log(x) + log(-x + 1) - 1 +1
//= -log(x) + log(-x + 1)
float entropyfuzzy_grad(float g, float *v) {
    double H = 0;
    for(int i = 0; i < layer1_size; ++i)  {
        float d = - entropylog(v[i])  - entropylog(1 - v[i]);
        v[i] = clamp01(v[i] + d); 
    }
    return H;
...
// >>> from sympy import *; x, y = var("x y"); h = -x * log(y) - (1 - x) * (1 - y);
// >>> h.diff(x)
// -y - log(y) + 1
// >>> h.diff(y)
// -x - x/y + 1
...
void klfuzzy_grad_left(float g, float *v, float *w, float *vgrad) {
    for(int i = 0; i < layer1_size; ++i) { 
        float d = - w[i] - entropylog(w[i]) + 1; vgrad[i] = clamp01(vgrad[i] + g * d);
    }
}

void klfuzzy_grad_right(float g, float *v, float *w, float *wgrad) {
    for(int i = 0; i < layer1_size; ++i) { 
        float d = -v[i] - v[i]/w[i] + 1; wgrad[i] = clamp01(wgrad[i] + g * d);
    }
}

/*
>>> xh = x * (log(x) - log(y)) + (1 - x) * (log (1 - x) - log(1-y))
>>> xh
x*(log(x) - log(y)) + (-x + 1)*(log(-x + 1) - log(-y + 1))
>>> simplify(xh.diff(x))
log(x) - log(y) - log(-x + 1) + log(-y + 1)
>>> simplify(xh.diff(y))
(x - y)/(y*(y - 1))
>>> 
*/
...
void crossentropyfuzzy_grad_left(float g, float *v, float *w, float *vgrad) {
    for(int i = 0; i < layer1_size; ++i)  {
        float d = log(v[i]) - log(w[i]) - log(1 - v[i]) + log(1 - w[i]);
        vgrad[i] = clamp01(vgrad[i] + g * d);
    }
}
void crossentropyfuzzy_grad_right(float g, float *v, float *w, float *wgrad) {
    for(int i = 0; i < layer1_size; ++i)  {
        float d = (v[i] - w[i])/(w[i] * (w[i] - 1));
        wgrad[i] = clamp01(wgrad[i] + g * d);
    }
}
...
void InitNet() {
    ...
    for (a = 0; a < vocab_size; a++) {
        for (b = 0; b < layer1_size; b++) {
            ...
            const int sign = ((next_random & 0xFFFF) / (real)65536) >= 0.5 ? 1 : -1; 
            next_random = next_random * (unsigned long long)25214903917 + 11;
            syn0[a * layer1_size + b] = clamp01(0.5 + sign * 0.5 * (((next_random & 0xFFFF) / (real)65536)));
        }
                    ...
                    if (negative >= 0) {
                        for (d = 0; d < negative + 1; d++) {
                            ...
                            */
                            // gradient
                            l2 = target * layer1_size;
                            // f = klfuzzy(syn0 +  l1, syn0 + l2);
                            f = klfuzzy(syn0 +  l2, syn0 + l1);
#ifdef DEBUG
                            assert(f >= 0);
//                             printf("f: %4.2f\n", f);
#endif
                            ...
                            // loss = [label - f]^2 + REGULARIZATION * [1.0 - H(focus)]^2 + [1.0 - H(ctx)]^2
                            g = ((label / (float)(2 * window)) - f) * alpha;
                            total_loss += g * g;
                            klfuzzy_grad_right(g, syn0 + l2, syn0 + l1, neu1e);
                            klfuzzy_grad_left(g, syn0 + l2, syn0 + l1, syn0 + l2);
                            diff_uniform_grad(syn0 + l2, alpha * REGULARIZATION, syn0 + l2);
                            // entropyfuzzy_grad(g, syn0 + l2);
                            // entropyfuzzy_grad(g, syn0 + l1);

                            // const float greg_l1 = REGULARIZATION * (1.0 - entropyfuzzy(syn0 + l1));
                            // entropyfuzzy_grad(greg_l1, syn0 + l1);

                            // const float greg_l2 = REGULARIZATION * (1.0 - entropyfuzzy(syn0 + l2));
                            // entropyfuzzy_grad(greg_l2, syn0 + l2);
                    ...
                    next_random =
                        ...
                        (unsigned long long)25214903917 +
                        11;
                    for (c = 0; c < layer1_size; c++) { syn0[c + l1] = clamp01(syn0[c + l1] +  neu1e[c]); }
                    diff_uniform_grad(syn0 + l1, alpha * REGULARIZATION, syn0 + l1);
                    // normalize(syn0 + l1);
                }
        }
        sentence_position++;
